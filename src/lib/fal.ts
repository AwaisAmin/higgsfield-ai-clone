...
/** Enqueue a generation. Returns fal's request id; does not wait for the result. */
export async function submitGeneration({
  model,
  prompt,
  aspectRatio,
}: FalSubmitInput): Promise<string> {
...
/**
 * Fetch a completed result. Throws if fal reports the request failed, which is
 * how a failed generation surfaces — there is no FAILED queue status.
 */
export async function getGenerationResult(
  ...
  return { imageUrl: images[0]?.url ?? null };
}
